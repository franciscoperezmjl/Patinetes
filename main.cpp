...
int main() {
    ...
    std::cout << "\n=== Ejemplos de Búsqueda ===" << std::endl;
    
    Patinete* resultado1 = arbol.buscar(123);
    if (resultado1 != nullptr) { %si lo encuentra muestra esto
        std::cout << "Búsqueda exitosa - Patinete ID 123: Estado = " << resultado1->estado << std::endl; 
    } else {
        std::cout << "Patinete ID 123 no encontrado." << std::endl;
    }
    
    Patinete* resultado2 = arbol.buscar(999); %este no lo encuentra entonces nos lo dice
    if (resultado2 != nullptr) {
        std::cout << "Búsqueda exitosa - Patinete ID 999: Estado = " << resultado2->estado << std::endl;
    } else {
